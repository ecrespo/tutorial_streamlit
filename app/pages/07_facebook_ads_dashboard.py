...
@st.cache_data
def get_data():
    df = pd.read_csv('app/pages/resources/fb_ads1.csv')
...
with total1:
    st.image('app/pages/resources/impression.png', use_column_width='Auto')
    st.metric(label='Total Impressions', value=numerize(total_impressions))

with total2:
    st.image('app/pages/resources/tap.png', use_column_width='Auto')
    st.metric(label='Total Clicks', value=numerize(total_clicks))

with total3:
    st.image('app/pages/resources/hand.png', use_column_width='Auto')
    st.metric(label='Total Spend', value=numerize(total_spent, 2))

with total4:
    st.image('app/pages/resources/conversion.png', use_column_width='Auto')
    st.metric(label='Total Conversions', value=numerize(total_conversions))

with total5:
    st.image('app/pages/resources/app_conversion.png', use_column_width='Auto')
    st.metric(label='Approved Conversions', value=numerize(total_approved_conversions))

Q1, Q2 = st.columns(2)

with Q1:
    tmp1 = df1.groupby(by=['campaign'])
    df3 = tmp1[['Impressions', 'Clicks']].sum().reset_index()
...
with Q3:
    tmp1 = df1.groupby(by='gender')
    df4 = tmp1[['Spent']].sum().reset_index()
...
with Q4:
    tmp1 = df1.groupby(by='age')
    df5 = tmp1[['Spent', 'Total_Conversion']].sum().reset_index()
    ...
    fig_CPC_by_age.update_layout(title={'x': 0.5}, xaxis=(dict(showgrid=False)), yaxis=(dict(showgrid=False)),
                                 plot_bgcolor="rgba(0,0,0,0)")
